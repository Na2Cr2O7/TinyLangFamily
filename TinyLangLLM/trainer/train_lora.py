    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if step % args.log_interval == 0:
            ...
            if math.isnan(loss.item()):
                x=input("loss=Nan,你还要继续训练吗？(y/n)")
                if x=="n":
                    raise SystemExit("loss=Nan")
                print("loss=Nan, 重新训练,但是这一次训练不会保存")
                continue
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0) and not  math.isnan(loss.item()):
...
def init_model(lm_config):
    ...
    # apply_lora(model)
    # load_lora(model, f'../out/lora/lora_N_{lm_config.hidden_size}.pth')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/lora_data.jsonl")
    parser.add_argument("--lora_name", type=str, default="lora_N", help="根据任务保存成lora_(英文/医学/心理...)")
    ...
    if not ddp or dist.get_rank() == 0:
        ...
        print(f"LLM 总参数量: {(total_params / 10**10) }B")
